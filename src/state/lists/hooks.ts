...
import DEFAULT_TOKEN_LIST from '@/config/constants/tokenLists/nbs-default.tokenlist.json'
import UNSUPPORTED_TOKEN_LIST from '@/config/constants/tokenLists/nbs-unsupported.tokenlist.json'
import WARNING_TOKEN_LIST from '@/config/constants/tokenLists/nbs-warning.tokenlist.json'
    ...
    return useMemo(
        () =>
            _pickBy(
                urls,
                (_, url) =>
                    (chainId === ChainId.SCROLL && ETH_URLS.includes(url))
            ),
        [chainId, urls],
    )
}
