...
import { ERC20Token, WETH } from '@/utils/token'
...
const PoolList = () => {
    ...
    const pairs = useMemo(() => (poolType === 'main' ? PINNED_PAIRS[ChainId.SCROLL] : allPairs), [poolType, allPairs])
    ...
    return (
        <Container sx={{ 
            ...
            <Box sx={{ display: 'flex', gap: 2, color:'#fff', flexDirection: { xs: 'column', md: 'row' } }}>
                <Tabs
                    value={poolType}
                                ...
                                if (page * 5 >= index + 1 && index + 1 > (page - 1) * 5)
                                    return (
                                        <StyledTableRow
                                            key={index}
                                            onClick={() => {
                                                if (pair.currencyIdA === WETH[ChainId.SCROLL].address)
                                                    navigate(`/add?currencyA=ETH&currencyB=${pair.currencyIdB}`)
                                                else if (pair.currencyIdB === WETH[ChainId.SCROLL].address)
                                                    navigate(`/add?currencyA=${pair.currencyIdA}&currencyB=ETH`)
                                                else
                                                    navigate(
