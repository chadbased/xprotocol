...
import { ChainId, scroll } from '@/config/constants/chains'
...
export function getBlockExploreLink(
    ...
    const chainId = chainIdOverride || ChainId.SCROLL
    const chain = chains.find((c) => c.id === chainId)
    if (!chain) return scroll.blockExplorers.default.url
    switch (type) {
        case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
    ...
    return chain?.blockExplorers?.default.name || scroll.blockExplorers.default.name
}
